...
`recipe_knowledge.csv` 採九欄可信內容結構：`ingredient`、`recipe_name`、`knowledge_id`、`steps`、`tips`、`source_name`、`verified_date`、`產地`、`有效期限`。目前無法驗證單一採購批次時，產地與期限會保守標示為依實際產品包裝及保存狀態判定，不虛構來源資料。
