...
React 正式版會自動依裝置時間判斷用餐時段、取得西屯預設天氣，定位後改用座標更新天氣與距離。內食的宅家、清冰箱、健身情境會實際套用不同食材、熱量、時間與缺料限制；食材可修改名稱、保存資訊、價格、易腐程度或刪除，窄螢幕下欄位會在側欄內自動縮放。登入後可點擊頁首愛心查看及移除收藏。

外食與內食的進階分析 Dashboard 具備手機版寬度邊界；摘要卡與分數拆解會縮放至畫面內，寬表格則只在表格容器內水平捲動，不會撐寬整個頁面。

`datasets/recipe_knowledge.csv` 採九欄可信內容結構：`ingredient`、`recipe_name`、`knowledge_id`、`steps`、`tips`、`source_name`、`verified_date`、`產地`、`有效期限`。目前無法驗證單一採購批次時，產地與期限會保守標示為依實際產品包裝及保存狀態判定，不虛構來源資料。

料理步驟採嚴格來源約束：候選食譜必須具有不重複的內容編號、料理步驟、來源名稱及審核日期，才可進入最終排序與 API 回傳。缺少可信內容的候選會被排除，系統不使用模型自行補寫步驟。
...
├── core/                     # 推薦、評論、食譜、食材與天氣核心
│   ├── recommender.py        # 外食基礎推薦
│   ├── review_score.py       # 評論風險核心
│   ├── review_analyzer.py    # 評論摘要與資料合併
│   ├── recipe_rank.py        # 食材召回與食譜基礎排序
│   ├── ingredient.py         # 食材優先級
│   ├── recipe_recommender.py # CSV 與可信知識轉接
│   └── weather_service.py    # 天氣情境
├── datasets/                 # 部署映像內的唯讀 CSV
│   ├── restaurants.csv
│   ├── reviews.csv
│   ├── recipes.csv
│   └── recipe_knowledge.csv
├── data/                     # Railway Volume 與執行時 SQLite
...
此展示程式直接匯入 `core/review_score.py`、`core/recipe_rank.py` 與 `core/ingredient.py`，和正式 API 使用相同核心公式，不另維護報告專用副本。
