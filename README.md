...
- **內食推薦**：根據冰箱現有食材、可接受烹飪時間、料理難度、熱量上限與缺少食材數推薦食譜。
...
- 提供本次推薦重點摘要，快速指出最高 CP 值、最近餐廳、最快食譜與食材最接近食譜。
...
外食推薦結果會以圖針標示在互動式地圖上。系統會依照推薦結果與 CP 值改變圖針顏色：
...
當使用者輸入的食材越接近食譜所需食材，推薦分數越高。系統也會顯示缺少食材數，協助使用者判斷是否能直接料理。

## 技術含量說明

本專題的技術核心不是單純查資料，而是把使用者條件轉換成可計算的推薦分數：

- 外食推薦使用多條件加權模型，綜合評分、價格、距離、類型、天氣、心情、外帶、辣度與出餐速度。
- 情緒策略會依照不同心情額外調整分數，例如省錢模式提高低價與高 CP 值餐廳，疲累模式提高近距離與快速出餐餐廳。
- CP 值使用評分與價格換算成 0 到 100 分區間，並用於排序與地圖圖針顏色。
- 內食推薦使用集合比對概念，計算使用者現有食材與食譜所需食材的符合數與缺少數。
